...
loss0 = []
lossf = []
tfs = jnp.array([0.1, 1, 10])
for tf in tfs:
    print(f'Iterations: {tf/0.01}')
    # create network
    spacing = 1
    net = pnm.network.make_cubic_network(shape=[10, 10, 10], spacing=spacing)
    ...
    # fit porosimetry
    start = time.time()
    D, loss = fcn.fit_porosimetry(D0, solver=dfx.Euler(), t_span=(0, tf), dt=0.01)
    stop = time.time()
...
jnp.save('../data/invasion-speed-iter-10by10by10-' + device, results)