...
def run_invasion(net, pressures):
    ...
    #FIXME: the function below should be updated
    def invasion(i, state):
        # breakout state
        saturation, actual_invasion_pressure = state
        # get invading pressure
        invading_pressure = pressures[i]
        # find all invaded throats using the actual invasion pressure
        invaded_throats = jnp.where(actual_invasion_pressure < invading_pressure)
        # find all invaded pores
        throats = jnp.where(invaded_throats, jnp.arange(Nt), -1)
        invaded_pores = pnm.models.find_neighbor_pores(net, throats)
    ...
    invaded_state = find_invasion_pressure(net)
    invaded_throats, invaded_pores, pressure, invasion_pressure  = invaded_state    
    # set initial state
    init_state = (saturation, invasion_pressure)
